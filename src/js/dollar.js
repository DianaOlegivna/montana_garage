export function dollar() { 
    ...
    const randomSvg = svgs[Math.floor(Math.random() * svgs.length)];
        ...
        for (let i = 0; i < 1; i++) { // Створюємо 5 іконок за один клік
            ...
            flyingDollar.style.transition = 'transform 3s ease-out, opacity 3s ease-out';
            flyingDollar.style.zIndex = 9999;

            // Анімація вгору
            setTimeout(() => {
                flyingDollar.style.transform = `translateY(-300px)`;
                flyingDollar.style.opacity = 0;
            }, 100);
